...
const AprovedRequest = () => {
    ...
    useEffect(() => {
        ...
        .then(res => res.json())
        .then(result => {
            const filterOnlyPending = result.filter(single => single?.requestStatus == 'Aprove')
                    ...
                    <tbody>
                        {
                            newEstate.map((single,indx) => (
                                <tr key={indx}className='hover:shadow-md' >
                                    ...
                                    <Link to={`aprove-details/${single?._id}`}  className='p-1 md:p-2  text-xl bg-yellow-600 text-white rounded' >Details</Link>
